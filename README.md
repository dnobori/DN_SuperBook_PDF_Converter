...
8. **高精細な日本語 OCR 処理を実施し、「文字埋め込み・検索可能 PDF」を生成。また、「Markdown」、「HTML」、「JSON」形式も合せて出力し、フルテキストインデックス作成や AI 分析処理等に利用可能に**  
   ...
   **単に YomiToku OCR ソフトウェアを呼び出しているだけではありません。快適性のために、面倒な読書用に必要な PDF 内部メタデータ再適用も行なっています。**YomiToku OCR ソフトウェアを呼び出すと、PDF に埋め込んだ前記のメタデータ (物理／論理ページ変換データなど) が消えてしまいます。本ツールは、YomiToku OCR ソフトウェアが生成する PDF に、再度、それらのメタデータを埋め込む処理を行ないます。これにより、上記で解説した快適な読書用 PDF で、かつ文字列が埋め込まれた検索可能 PDF を、自動出力することができます。
