...
## 8.1. 派生ソフトウェア
- **[superbook-pdf](https://github.com/mikkegt/superbook-pdf)** — Python 書き換え版。macOS (Apple Silicon) で開発・テスト済み。Linux・Windows でも動作する見込み。元の C# 版と同じ 6 ステップパイプライン（傾き補正、色補正、AI 超解像、テキスト領域検出、IQR クロップ、PDF 再構築）を Python (OpenCV, numpy) で再実装。Homebrew 等のパッケージマネージャで依存ツールをインストールするだけで利用可能。
